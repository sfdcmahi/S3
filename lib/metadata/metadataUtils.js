...
function metadataValidateBucketAndObj(params, log, callback) {
    ...
    async.waterfall([
        ...
        function handleNullVersionGet(bucket, objMD, next) {
            ...
            const vStatus = vCfg ? vCfg.Status : undefined;
            const overwritingNullVersion = vStatus === 'Suspended' &&
            ['objectPut', 'putPart or complete'].includes(requestType);
            if (objMD && (versionId === 'null' || overwritingNullVersion)) {
                return getNullVersion(objMD, bucketName, objectKey, log,
                    (err, nullVer) => next(err, bucket, nullVer));
            }
            return process.nextTick(next, null, bucket, objMD);
        },
